...
function KGPage() {
  const [selected, setSelected] = useState<GraphNode>(
    graphNodes.find((n) => n.id === "entropy")!,
  );
  ...
  const related = graphEdges
    ...
    .map(([a, b]) =>
      graphNodes.find((n) => n.id === (a === selected.id ? b : a))!,
    )
    .filter(Boolean);
      ...
      <div className="soft-card p-4 mb-4 flex items-center justify-between flex-wrap gap-3">
        <div className="relative">
          <select
            ...
            className="appearance-none pl-4 pr-10 py-2 rounded-xl bg-white/5 border border-white/10 text-[13px] focus:outline-none"
          >
            <option>All</option>
        ...
        <div className="flex items-center gap-2">
          <button className="p-2 rounded-lg border border-white/10 hover:border-white/25">
            <ZoomIn className="w-4 h-4" />
          </button>
          <button className="p-2 rounded-lg border border-white/10 hover:border-white/25">
            <ZoomOut className="w-4 h-4" />
          </button>
          <button className="px-3 py-2 rounded-lg border border-white/10 text-[12px] hover:border-white/25 flex items-center gap-1.5">
            ...
            {graphEdges.map(([a, b], i) => {
              ...
              return (
                <line
                  ...
                  x2={nb.x}
                  y2={nb.y}
                  stroke={active ? "#0000EE" : "rgba(255,255,255,0.12)"}
              ...
              <div className="flex flex-wrap gap-1.5">
                {related.map((n) => (
                  <button
                    key={n.id}
                    onClick={() => setSelected(n)}
                    className="px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-[11px] hover:border-white/25"
                  >
                    {n.label}
                  </button>
                ))}
