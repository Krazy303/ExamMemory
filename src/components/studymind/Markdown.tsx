        ...
        switch (tok.type) {
          case "image":
            return (
              <img
                ...
                src={tok.text}
                alt={tok.alt}
                className="max-w-full rounded-xl border border-border my-2 inline-block"
              />
            );
          case "link":
            return (
          ...
          case "code":
            return (
              <code
                key={i}
                className="px-1.5 py-0.5 rounded bg-muted border border-border font-mono text-[12px] text-foreground/95"
              >
                {tok.text}
              </code>
            );
          case "bolditalic":
            return (
              <strong key={i} className="font-bold italic text-foreground">
                {tok.text}
              </strong>
            );
          case "bold":
          ...
          case "strike":
            return (
              <span key={i} className="line-through text-foreground/40">
                {tok.text}
              </span>
            );
          case "mark":
            return (
              <mark
                key={i}
                className="bg-primary/30 text-foreground rounded px-1.5 py-0.5"
              >
                {tok.text}
              </mark>
            );
  ...
  parts.forEach((part, pIdx) => {
    if (part.startsWith("```") && part.endsWith("```")) {
      ...
      blockElements.push(
        <div
          key={`codeblock-${pIdx}`}
          className="my-4 rounded-xl border border-border bg-muted/40 p-4 font-mono text-[13px] text-foreground/90 overflow-x-auto"
        >
      ...
      const flushTable = (key: number) => {
        ...
        if (cleanRows.length > 0) {
          ...
          blockElements.push(
            <div
              key={`table-${key}`}
              className="overflow-x-auto my-3 rounded-xl border border-border bg-muted/[0.05]"
            >
        ...
        if (isTableLine) {
          ...
          if (trimmed.startsWith("> ")) {
            blockElements.push(
              <blockquote
                key={`bq-${idx}`}
                className="border-l-4 border-primary/50 pl-4 py-1 italic text-foreground/70 my-3 bg-muted/20 rounded-r-lg"
              >
          ...
          // Headings (1 to 6)
            ...
            blockElements.push(
              <h6
                key={`h6-${idx}`}
                className="text-[12px] font-bold text-foreground/70 mt-2 mb-1 uppercase tracking-wider"
              >
            ...
            blockElements.push(
              <h5
                key={`h5-${idx}`}
                className="text-[13px] font-bold text-foreground/80 mt-2 mb-1 uppercase tracking-wider"
              >
            ...
            blockElements.push(
              <h4
                key={`h4-${idx}`}
                className="text-[14px] font-bold text-foreground mt-3 mb-1"
              >
            ...
            blockElements.push(
              <h3
                key={`h3-${idx}`}
                className="text-[15px] font-bold text-foreground mt-3 mb-1"
              >
            ...
            blockElements.push(
              <h2
                key={`h2-${idx}`}
                className="text-[17px] font-bold text-foreground mt-4 mb-2"
              >
            ...
            blockElements.push(
              <h1
                key={`h1-${idx}`}
                className="text-[20px] font-bold text-foreground mt-5 mb-2"
              >
                {parseInline(trimmed.slice(2))}
              </h1>,
            );
            ...
            blockElements.push(
              ...
              >
                <input
                  ...
                  checked
                  disabled
                  className="rounded border-border bg-muted text-primary focus:ring-0 focus:ring-offset-0 w-3.5 h-3.5"
                />
                <span className="flex-1 text-foreground/40 line-through">
                  {parseInline(trimmed.slice(6))}
                </span>
              </div>,
            );
          ...
          // Default Paragraph
            ...
            blockElements.push(
              <p
                key={`p-${idx}`}
                style={plStyle}
                className="text-[14px] min-h-[1.2em] leading-relaxed text-foreground/90 my-1"
              >
                {parseInline(line)}
              </p>,
            );
  ...
  return (
    <div className="space-y-1">
      ...
      {footnoteDefs.length > 0 && (
        <div className="mt-4 pt-4 border-t border-border space-y-1.5 text-[11px] text-[color:var(--link)]">
